...
protocol CountrySelectedDelegate {
    func SRcountrySelected(countrySelected country: SRCountry,countryFlag: UIImage) -> Void
}
...
class SRCountryPickerController: UIViewController {
  ...
  var countriesFiltered = [SRCountry]()
  var countriesModel = [SRCountry]()
...
//      print("country list \(countries)")
  ...
  func collectCountries() {
    for country in countries  {
      ...
      countriesModel.append(SRCountry(country_code:code,dial_code:dailcode, country_name:name))
    }
  }
...
extension SRCountryPickerController: UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    if checkSearchBarActive() {
        countryDelegate.SRcountrySelected(countrySelected: countriesFiltered[indexPath.row],countryFlag : (cell?.imageView?.image)! )
    }else {
        countryDelegate.SRcountrySelected(countrySelected: countriesModel[indexPath.row],countryFlag : (cell?.imageView?.image)! )
    }
